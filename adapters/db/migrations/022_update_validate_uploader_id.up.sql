...
BEGIN
  ...
  ELSIF NEW.uploader_type = 'DIAGNOSTIC_CENTRE_MANAGER' THEN
    IF NOT EXISTS (SELECT 1 FROM diagnostic_centres WHERE id = NEW.uploader_id) THEN
      RAISE EXCEPTION 'uploader_id % does not exist in diagnostic_centres table', NEW.uploader_id;
    END IF;
  END IF;
  RETURN NEW;
END;
